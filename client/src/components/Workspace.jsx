...
import {
  ...
  Terminal as TerminalIcon, Eye, Save, GitCommit, ExternalLink,
  GitBranch, Loader2, CheckCircle, AlertCircle,
} from 'lucide-react';

const MIN_PANEL = 200;

const Workspace = () => {
  const { activeProject, files, setFiles, activeFile, setActiveFile, setFileTree } = useApp();
  ...
  useEffect(() => {
    if (activeProject) {
      if (activeProject.type === 'import') loadRepoTree();
        ...
        // Set files from template
        setFiles(activeProject.initialFiles);
        // Set active file to first one
  ...
  const loadRepoTree = async () => {
    ...
    try {
      ...
      // Auto-open README or index file
  ...
  const handleCommit = async () => {
    ...
    if (modifiedFiles.length === 0) {
      writeTerminal('[COMMIT] No modified files to commit.', 'info');
      return;
    }

    setCommitStatus('saving');
    writeTerminal(`[COMMIT] Pushing ${modifiedFiles.length} file(s) to GitHub...`, 'info');
    try {
      ...
      writeTerminal(`[COMMIT] ✅ Committed: ${data.sha?.slice(0, 7)}`, 'success');
      // Mark files as saved
      setFiles(prev => {
        const next = { ...prev };
      ...
      writeTerminal(`[COMMIT] ❌ ${err.response?.data?.error || err.message}`, 'error');
  ...
  const onResizeLeft = useCallback((e) => {
    ...
    const move = (ev) => setLeftWidth(Math.max(MIN_PANEL, Math.min(500, startW + ev.clientX - startX)));
  ...
  const onResizeRight = useCallback((e) => {
    ...
    const move = (ev) => setRightWidth(Math.max(MIN_PANEL, Math.min(600, startW - (ev.clientX - startX))));
    const up = () => { window.removeEventListener('mousemove', move); window.removeEventListener('mouseup', up); };
    window.addEventListener('mousemove', move);
    window.addEventListener('mouseup', up);
  }, [rightWidth]);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* Top toolbar */}
      <div className="flex items-center h-9 px-3 bg-omni-surface border-b border-omni-border shrink-0 gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 text-xs text-omni-muted font-mono truncate">
          {activeProject && (
            <>
          ...
          {loadingTree && <Loader2 className="w-3 h-3 animate-spin ml-2" />}
        </div>

        {/* Modified indicator + commit */}
        {modifiedCount > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-omni-yellow bg-omni-yellow/10 border border-omni-yellow/20 px-2 py-0.5 rounded">
              {modifiedCount} modified
            </span>
            {activeProject?.type !== 'local' && (
              <button
                onClick={() => setShowCommitBar(p => !p)}
                className="flex items-center gap-1.5 text-xs omni-btn-primary py-1 px-2.5"
              >
                {commitStatus === 'saving' ? (
        ...
        {pagesUrl && (
          <a href={pagesUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1.5 text-xs text-omni-green bg-omni-green/10 border border-omni-green/20 px-2.5 py-1 rounded-lg hover:bg-omni-green/20 transition-colors">
      ...
      {showCommitBar && (
        <div className="flex items-center gap-2 px-3 py-2 bg-omni-surface border-b border-omni-border shrink-0">
          ...
          <input
            ...
            placeholder="Commit message (Enter to push)"
            className="omni-input flex-1 h-7 py-1 text-xs"
            autoFocus
          />
          <button onClick={handleCommit} disabled={commitStatus === 'saving'}
            className="omni-btn-primary text-xs py-1 px-3 disabled:opacity-50">
            Push
          </button>
          <button onClick={() => setShowCommitBar(false)}
            className="text-omni-muted hover:text-omni-text text-xs px-2">✕</button>
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {showLeft && (
          <>
            <div style={{ width: leftWidth, minWidth: MIN_PANEL }} className="flex-shrink-0 flex flex-col overflow-hidden border-r border-omni-border bg-omni-surface">
              <FileTree
              ...
              {[
                ...
                <button key={tab.id} onClick={() => setBottomTab(tab.id)}
                  className={`flex items-center gap-1.5 px-3 py-1 rounded text-xs font-medium transition-colors
                    ${bottomTab === tab.id ? 'bg-omni-bg text-omni-text border border-omni-border'
        ...
        {/* RIGHT: AI Chat */}
        {showRight && (
          <div style={{ width: rightWidth, minWidth: MIN_PANEL }} className="flex-shrink-0 border-l border-omni-border overflow-hidden">
            <AIChat
              writeTerminal={writeTerminal}
